...
public class NetworkedHealth : NetworkBehaviour {
    ...
    public delegate void DamageDelegate(DamageArgs args);
    ...
    [SyncEvent]
    public event DamageDelegate EventDamaged;
    public static event DamageDelegate Damaged;
    ...
    private GameObject CheckForNetworkIdentity(GameObject obj)
    {
        return (obj != null && obj.GetComponent<NetworkIdentity>()) ? obj : null;
    }

    public void ChangeHealth(float changeAmount, GameObject source, bool autoReportDamage = true)
    {
        ...
        source = CheckForNetworkIdentity(source);
        CmdChangeHealth(changeAmount, source);
        if (changeAmount < 0 && autoReportDamage)
            CmdReportDamage(changeAmount, source);
    }

    public void SetHealth(float value, GameObject source)
    {
        CmdSetHealth(value, CheckForNetworkIdentity(source));
    }
    ...
    public void ReportDamage(float damageAmount, GameObject source)
    {
        CmdReportDamage(damageAmount, CheckForNetworkIdentity(source));
    }

    [Command]
    private void CmdReportDamage(float damageAmount, GameObject source)
    {
        damageAmount = Mathf.Abs(damageAmount);
        EventDamaged(new DamageArgs(this, source, damageAmount));
    }
    ...
    private void OnEnable()
    {
        ...
        EventDamaged += CallStaticDamaged;
    }
...
public struct DamageArgs
{
    ...
    public DamageArgs(NetworkedHealth sender, GameObject damageSource, float damageAmount)
    {
        senderObject = sender.gameObject;
        _sender = sender;
